...
import { BIN_NAME, PKG_VERSION } from "./package.ts";
...
export class Printer {
	...
	print(declarations: Declaration[]): string {
		...
		chunks.push(`// @generated by ${BIN_NAME} ${PKG_VERSION}\n\n`);
	...
	printType(decl: TypeDeclaration): string {
		return [
			decl.comment ? this.printDocComment(decl.comment) : "",
			`export type ${decl.name} = ${decl.type}${this.#semi}\n`,
		].join("");
	}
	...
	printInterface(decl: InterfaceDeclaration): string {
		...
		for (const prop of decl.properties) {
			chunks.push(
				decl.comment ? this.printDocComment(decl.comment, indent) : "",
	...
	printDocComment(text: string, indent: string = ""): string {
		if (!text.trim()) {
			return "";
		}
